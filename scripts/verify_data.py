    ...
    try:
        ...
        stats = await conn.fetch("""
            SELECT
                ...
                MIN(wt.tx_timestamp) as earliest_tx,
                MAX(wt.tx_timestamp) as latest_tx
            FROM blockchains b
        ...
        if eth_txs:
            ...
            for i, tx in enumerate(eth_txs, 1):
                ...
                logger.info(f"      金額: {tx['amount']:.4f} {tx['token_symbol'] or 'ETH'}")
                if tx['amount_usd']:
                    logger.info(f"      美元: ${tx['amount_usd']:,.2f}")
                logger.info(f"      大額: {tx['is_whale']}, 異常: {tx['is_anomaly']}")
        ...
        total_addresses = await conn.fetchval("SELECT COUNT(*) FROM whale_addresses")

        logger.info(f"總交易數: {total_txs}")
        logger.info(f"總地址數: {total_addresses}")
